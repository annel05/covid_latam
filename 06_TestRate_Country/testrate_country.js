async function TestRateCountry(_country) {
  ...
  const width = document.getElementById('wrapper_testrate_main').parentElement
    .clientWidth;
  let dimensions = {
    width: width,
    height: 600,
  ...
  bounds
    ...
    .attr('class', d => `${d.key}_test rate states`);

  const tooltipLine = bounds
    .append('line')
    .attr('class', '.tooltipLine_testrate');
  ...
  const addStateLine = _stateCode => {
    ...
    bounds
      .append('path')
      .attr('class', `${_stateCode}_temp_testrate active_testrate`)
      .attr('fill', 'none')
  ...
  state_list
    .append('input')
    .attr('class', 'input_box_testrate')
    .attr('type', 'checkbox')
    .attr('name', d => `${stateCodeAccessor(d.values[0])}_testrate`);

  state_list
    .append('label')
    .attr('class', 'input_label')
    .attr('for', d => `${stateCodeAccessor(d.values[0])}_testrate`)
    .html(d => stateAccessor(d.values[0]));

  state_list
    .select(`[name=${firstRankCode}_testrate]`)
    .property('checked', true);
  state_list
    .select(`[for=${firstRankCode}_testrate]`)
    .style('color', colorScale(firstRankCode))
    .style('font-weight', 'bold');

  state_list
    .select(`[name=${lastRankCode}_testrate]`)
    .property('checked', true);
  state_list
    .select(`[for=${lastRankCode}_testrate]`)
    .style('color', colorScale(lastRankCode))
    .style('font-weight', 'bold');

  d3.selectAll('.input_box_testrate').on('input', toggleStateLine);
  ...
  function toggleStateLine() {
    ...
    if (this.checked) {
      ...
      bounds.select(`.${code}_temp_testrate`).remove();
  ...
  const tooltipDate = bounds
    .append('text')
    .attr('class', 'tooltipDate_testrate')
    .style('opacity', 0);
  ...
  const tooltip = d3
    .select('#tooltip_testrate')
    .style('opacity', 0)
    .style('top', `${dimensions.margin.top * 2}px`)
    .style('left', `${dimensions.margin.left * 1.25}px`);
  const tooltipHeader = tooltip.select('#tooltipHeader_testrate');
  const tooltipContent = tooltip.select('#tooltipContent_testrate');
  let activeStates;
  ...
  function onMouseMove() {
    ...
    const allActive = document
      .getElementById('wrapper_testrate_main')
      .getElementsByClassName('active_testrate');
    ...
    d3.selectAll('.temp_circle_testrate').remove();
    ...
    activeStates.forEach(element => {
      ...
      bounds
        ...
        .attr('class', 'temp_circle_testrate');
    });
  ...
  function onMouseLeave() {
    ...
    bounds.selectAll('.temp_circle_testrate').remove();
